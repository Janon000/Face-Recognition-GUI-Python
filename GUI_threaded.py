...
import cv2
import time
import face_recognizer_threaded, faces_recognizer_og
import imutils
...
def update_gui(frame,name,locations):
    global image
    if frame is not None:
        if locations:
    ...
    photo.paste(image)
    root.after(round(40), update_gui)  # update displayed image after: round(1000/FPS) [in milliseconds]
...
#change this take out if?
if __name__ == '__main__':
    threaded_camera = Threaded_Camera()
    if RECOGNIZE:
        while threaded_camera.is_active():
            threaded_camera.do_recognition()
